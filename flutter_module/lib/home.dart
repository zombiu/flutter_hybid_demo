...
class _HomePageState extends State<HomePage> {
  ...
  static const MethodChannel _moblinkRestoreSceneChannel = const MethodChannel('MOBLINK_RESTORE_SCENE');
  ...
  void initState() {
    ...
    Moblink.restoreScene((MLSDKScene scene) {
      showAlert('restoreScene要还原的路径为：' + scene.className, context);
      print('restoreScene要还原的路径为：' + scene.className);
    });
    ...
    //moblink 监听开始(用户监听场景还原的数据回传回来) android
    if (Platform.isAndroid) {
      _moblinkRestoreSceneChannel.setMethodCallHandler((MethodCall call) async {
        debugPrint("-->>moblink场景还原 : ${call.arguments}");
        if (call.method == "restore_scene_call") {
          String url = call.arguments["href"] ?? "";

...
class MoblinkPluginKotlin(private val activity: Activity) : MethodCallHandler, SceneRestorable {
    ...
    class SceneListener : RestoreSceneListener {
        override fun willRestoreScene(scene: Scene): Class<out Activity>? {
            ...
            //android端场景还原 需要将具体参数传递给flutter
            moblinkToFlutter.invokeMethod("restore_scene_call", scene.getParams(), object : MethodChannel.Result {
                override fun success(result: Any?) {
                    Log.e("-->>", " implemented success")
                }

                override fun error(errorCode: String?, errorMessage: String?, errorDetails: Any?) {
                    Log.e("-->>", " implemented error $errorCode $errorMessage $errorDetails")
                }

                override fun notImplemented() {
                    Log.e("-->>", " implemented notImplemented")
                }
    ...
    companion object {
        ...
        lateinit var moblinkToFlutter: MethodChannel
        ...
        fun registerWith(flutterEngine: FlutterEngine, activity: Activity) {
            ...
            //restoreScene
            moblinkToFlutter = MethodChannel(dartExecutor, "MOBLINK_RESTORE_SCENE");
            moblinkToFlutter.setMethodCallHandler(moblinkPluginKotlin::onMethodCall)
